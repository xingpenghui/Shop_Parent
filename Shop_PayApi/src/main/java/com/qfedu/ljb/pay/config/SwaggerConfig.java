package com.qfedu.ljb.pay.config;
...
public class SwaggerConfig {
    ...
    public ApiInfo createA(){
        ApiInfo info=new ApiInfoBuilder().title("统一支付中心-数据接口平台").
                contact( new Contact("邢朋辉","http://www.mobiletrain.org","xingfei_work@163.com")).
                description("实现一个统一支付中心，支持微信支付和支付宝支付").build();
        return info;
    }
    /**
     * 创建API应用
    ...
    public Docket createDocket(){
       Docket docket=new Docket(DocumentationType.SWAGGER_2).apiInfo(createA()).select().
               apis(RequestHandlerSelectors.basePackage("com.qfedu.ljb.pay.controller")).
               build();
       return docket;
    }
}